...
- **Storage & Data**:
  ...
  - **Queues** (paid feature): OCR/metadata ingestion, push fan-out, anti-cheat aggregation. Note: Cloudflare Queues are a paid feature and are not available on the free plan; the project defaults to free-plan-compatible alternatives (see "Background processing on the free plan").
...
3. Create **D1** database and apply migrations:
   ...
   ```
4. Configure **wrangler.toml** bindings (D1/KV/R2, VAPID, Turnstile). Note: Cloudflare Queues are paid — do not rely on queue bindings for free-plan deployments.
...
3. Add **Environment Variables & Bindings**:
  - `DB_D1` (D1), `KV_APP` (KV), `R2_FILES` (R2)
...
- Users store **Web Push** subscriptions per device.
 - Rehber announcements enqueue messages to a background mechanism (e.g., D1/KV-backed task store or external runner); on the paid plan you can switch to **PUSH_QUEUE** for fan-out.
...
## Background processing on the free plan

Cloudflare Queues are a paid feature. This project avoids depending on Queues so the repository and `wrangler.toml` remain compatible with the free Cloudflare plan. Recommended free-plan alternatives:

- Task store + cron Workers: push small task objects into KV or a D1 table and use a scheduled Worker (cron trigger) to process batches periodically.
- Inline/short-run Workers: perform quick processing synchronously on upload (keep work short to avoid timeouts) and persist intermediate state to D1/R2/KV for later processing.
- External runners: run heavier jobs on external free-tier services (GitHub Actions, Render free jobs) and call your Cloudflare API endpoints to write results back.

When you upgrade to a paid Cloudflare plan, you can reintroduce Queues for more reliable fan-out and scale.
